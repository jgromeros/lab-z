...
<div class="layerder">
	<p>Estos son los niveles en el banco ${model.bankPlace.name }</p>
	<table border="1">
		<tr>
			<th>Nombre</th>
			<th>Descripci&oacute;n</th>
		</tr>
		<c:forEach var="level" items="${model.bankPlace.places }">
			<tr>
				<td><c:out value="${level.name }"/></td>
				<td><c:out value="${level.longDescription }"/></td>
				<td><a href="bankform.htm?id=${level.id }">Editar</a></td>
			</tr>
		</c:forEach>
	</table>
	<c:if test="${model.bankPlace.level == 0}">
		<p><a href="bankform.htm?parent=${model.bankPlace.id }">Crear Niveles</a></p>
	</c:if>
